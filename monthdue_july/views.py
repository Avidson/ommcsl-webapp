...
@login_required
def julyDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = JulyDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_july:payment-process"))

    except SystemError as e:
        pass
    july_payment = JulyDue.objects.filter(client_name=request.user, paid=True)

    context = {'july_payment': july_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(JulyDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_july:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_july:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(JulyDue, client_name=request.user)#new
