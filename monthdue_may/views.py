...
@login_required
def mayDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = MayDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_may:payment-process"))

    except SystemError as e:
        pass
    may_payment = MayDue.objects.filter(client_name=request.user, paid=True)

    context = {'may_payment': may_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(MayDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_may:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_may:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(MayDue, client_name=request.user)#new
