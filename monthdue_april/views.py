...
@login_required
def aprilDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = AprilDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_april:payment-process"))

    except SystemError as e:
        pass
    august_payment = AprilDue.objects.filter(client_name=request.user, paid=True)
...
def payment_process(request):
    ...
    payment = get_object_or_404(AprilDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_april:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_april:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(AprilDue, client_name=request.user)#new
