...
@login_required
def septemberDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = SeptemberDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_september:payment-process"))

    except SystemError as e:
        pass
    september_payment = SeptemberDue.objects.filter(client_name=request.user, paid=True)

    context = {'september_payment': september_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(SeptemberDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_september:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_september:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(SeptemberDue, client_name=request.user)#new
