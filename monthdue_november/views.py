...
@login_required
def novemberDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = NovemberDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_november:payment-process"))

    except SystemError as e:
        pass
    november_payment = NovemberDue.objects.filter(client_name=request.user, paid=True)

    context = {'november_payment': november_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(NovemberDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_november:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_november:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(NovemberDue, client_name=request.user)#new
