...
@login_required
def octoberDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = OctoberDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_october:payment-process"))

    except SystemError as e:
        pass
    october_payment = OctoberDue.objects.filter(client_name=request.user, paid=True)

    context = {'october_payment': october_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(OctoberDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_october:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_october:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(OctoberDue, client_name=request.user)#new
