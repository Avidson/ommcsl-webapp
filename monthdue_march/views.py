...
@login_required
def marchDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = MarchDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_march:payment-process"))

    except SystemError as e:
        pass
    march_payment = MarchDue.objects.filter(client_name=request.user, paid=True)

    context = {'march_payment': march_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(MarchDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_march:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_march:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(MarchDue, client_name=request.user)#new
