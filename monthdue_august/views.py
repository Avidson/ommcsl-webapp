...
@login_required
def augustDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = AugustDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_august:payment-process"))

    except SystemError as e:
        pass
    august_payment = AugustDue.objects.filter(client_name=request.user, paid=True)

    context = {'august_payment': august_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(AugustDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_august:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_august:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(AugustDue, client_name=request.user)#new
