...
@login_required
def decemberDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = DecemberDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_december:payment-process"))

    except SystemError as e:
        pass
    december_payment = DecemberDue.objects.filter(client_name=request.user, paid=True)

    context = {'december_payment': december_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(DecemberDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_december:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_december:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(DecemberDue, client_name=request.user)#new
