...
@login_required
def juneDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = JuneDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_june:payment-process"))

    except SystemError as e:
        pass
    june_payment = JuneDue.objects.filter(client_name=request.user, paid=True)

    context = {'june_payment': june_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(JuneDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_june:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_june:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(JuneDue, client_name=request.user)#new
