...
@login_required
def februaryDue(request, *args, **kwargs):
    ...
    try:
        ...
        if request.method == 'POST' :
            ...
            monthly_payment = FebruaryDue.objects.create( 
            ...
            if monthly_payment:
                ...
                return redirect(reverse("monthdue_february:payment-process"))

    except SystemError as e:
        pass
    february_payment = FebruaryDue.objects.filter(client_name=request.user, paid=True)

    context = {'february_payment': february_payment}
...
def payment_process(request):
    ...
    payment = get_object_or_404(FebruaryDue, id=payment_id)
    ...
    if request.method == 'POST' :
        ...
        success_url = request.build_absolute_uri(
            reverse('monthdue_february:payment-success')
        )
        cancel_url = request.build_absolute_uri(
            reverse('monthdue_february:payment-cancelled')
        )
...
def payment_success(request):
    ...
    payment = get_object_or_404(FebruaryDue, client_name=request.user)#new
